...
export class AdminAuditService {
   ...
   */
  static logProductAction(adminUser, action, productId, productName, changes = {}) {
    if (!adminUser) {
  ...
  static logFailedAuth(email, reason, ip) {
    const logData = { email, reason, timestamp: new Date().toISOString(), ip };
...
export const auditMiddleware = (req, res, next) => {
  ...
  // Override methods để include IP
  AdminAuditService.logUserAction = (adminUser, action, targetUserId, changes = {}) => {
    if (!adminUser) return;
    const logData = { ...{ adminId: adminUser.id, adminEmail: adminUser.email, adminRole: adminUser.role, action, targetUserId, changes }, timestamp: new Date().toISOString(), ip: req.ip || req.connection.remoteAddress };
    logger.info('ADMIN_USER_ACTION', logData);
  };

  AdminAuditService.logProductAction = (adminUser, action, productId, productName, changes = {}) => {
    if (!adminUser) return;
    const logData = { ...{ adminId: adminUser.id, adminEmail: adminUser.email, adminRole: adminUser.role, action, productId, productName, changes }, timestamp: new Date().toISOString(), ip: req.ip || req.connection.remoteAddress };
    logger.info('ADMIN_PRODUCT_ACTION', logData);
  };

  AdminAuditService.logOrderAction = (adminUser, action, orderId, orderCode, changes = {}) => {
    const logData = { ...{ adminId: adminUser.id, adminEmail: adminUser.email, adminRole: adminUser.role, action, orderId, orderCode, changes }, timestamp: new Date().toISOString(), ip: req.ip || req.connection.remoteAddress };
    logger.info('ADMIN_ORDER_ACTION', logData);
  };

  AdminAuditService.logReviewAction = (adminUser, action, reviewId, userId, productId) => {
    const logData = { ...{ adminId: adminUser.id, adminEmail: adminUser.email, adminRole: adminUser.role, action, reviewId, userId, productId }, timestamp: new Date().toISOString(), ip: req.ip || req.connection.remoteAddress };
    logger.info('ADMIN_REVIEW_ACTION', logData);
  };

  AdminAuditService.logDashboardAccess = (adminUser, endpoint, filters = {}) => {
    const logData = { ...{ adminId: adminUser.id, adminEmail: adminUser.email, adminRole: adminUser.role, action: 'DASHBOARD_ACCESS', endpoint, filters }, timestamp: new Date().toISOString(), ip: req.ip || req.connection.remoteAddress };
    logger.info('ADMIN_DASHBOARD_ACCESS', logData);
  };
